    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 80% at 50% 90%, transparent 30%, rgba(153, 27, 27, 0.45) 70%, rgba(8, 9, 10, 0.95) 100%)",
          }}
        />

        <div className="pointer-events-none absolute left-6 top-24 text-iron-red md:left-10 md:top-28 animate-pulse-iron">
          <HudFrame corner="tl" size={26} />
        </div>
        <div className="pointer-events-none absolute right-6 top-24 text-iron-red md:right-10 md:top-28 animate-pulse-iron">
          <HudFrame corner="tr" size={26} />
        </div>
        <div className="pointer-events-none absolute bottom-14 left-6 text-iron-red md:bottom-16 md:left-10 animate-pulse-iron">
          <HudFrame corner="bl" size={26} />
        </div>
        <div className="pointer-events-none absolute bottom-14 right-6 text-iron-red md:bottom-16 md:right-10 animate-pulse-iron">
        ...
        <div className="pointer-events-none absolute right-6 top-28 z-10 flex max-w-[46ch] flex-col items-end gap-5 text-right md:right-12 md:top-32">
          <EyebrowBadge className="border-iron-red/30 text-iron-red bg-iron-red/5">AVENGERS ENDGAME // THE SACRIFICE</EyebrowBadge>
          <div className="relative self-stretch">
            <h2
              ref={h2InevitableRef}
              className="font-heading text-5xl leading-[0.9] tracking-tighter text-foreground md:text-7xl lg:text-8xl"
              style={{ transition: "opacity 240ms ease-out" }}
            >
              The End
              <br />
              <span className="text-iron-red">of an Era.</span>
            </h2>
            <h2
              ref={h2IronManRef}
              className="absolute inset-0 font-heading text-5xl leading-[0.9] tracking-tighter text-foreground md:text-7xl lg:text-8xl"
              style={{ opacity: 0, transition: "opacity 240ms ease-out" }}
            >
              I am
              <br />
              <span className="text-iron-red shimmer-iron">Iron Man.</span>
            </h2>
          </div>
          <p className="max-w-[42ch] font-sans text-sm leading-relaxed text-zinc-400 md:text-base">
            Tony Stark's journey comes to a close. A hero's final stand that saved the multiverse
            from total annihilation.
          </p>
        </div>

        <div className="pointer-events-none absolute left-6 top-20 z-10 flex items-center gap-2 md:left-10 md:top-24">
          <div className="h-px w-8 bg-iron-red/60" />
        ...
        <div className="pointer-events-none absolute right-6 top-20 z-10 flex items-center gap-3 md:right-10 md:top-24">
          <span
            ref={seqReadoutRef}
            className="font-mono text-[10px] uppercase tracking-[0.28em] text-iron-gold"
          >
            SEQ 001 / {CINE_FRAME_COUNT}
          </span>
          <span
            aria-hidden
            className="inline-block h-1.5 w-1.5 rounded-full bg-iron-red shadow-[0_0_10px_rgba(239,68,68,0.85)]"
          />
        </div>
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10">
          <div className="mx-6 mb-3 h-px bg-white/10 md:mx-10">
            <div
              ref={progressFillRef}
              className="h-full origin-left bg-iron-red"
              style={{ transform: "scaleX(0)", transition: "transform 80ms linear" }}
            />
          </div>
          <div className="mx-6 flex items-center justify-between pb-4 font-mono text-[10px] uppercase tracking-[0.28em] text-zinc-500 md:mx-10">
            <span>TONY STARK // LEGACY</span>
        ...
        {BEATS.map((b, i) => {
          ...
          return (
            ...
            >
              <figure
                className={`card-surface pointer-events-auto p-6 transition-all duration-400 ease-out border-iron-red/10 ${
                  visible ? "translate-y-0 opacity-100" : "translate-y-5 opacity-0"
                }`}
              >
                <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-iron-gold">
                  {b.label}
                </span>
                ...
                <figcaption className="mt-4 flex items-center justify-between">
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-[0.24em] text-iron-red/80">
                    {b.film}
                  </span>
                </figcaption>
              </figure>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-[36%] z-20 flex flex-col gap-3 px-6 md:hidden">
          {BEATS.map((b) => {
            ...
            return (
              <figure
                key={b.id}
                className={`card-surface pointer-events-auto p-5 transition-all duration-400 ease-out border-iron-red/10 ${
                  visible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
                }`}
              >
                <span className="font-mono text-[9px] uppercase tracking-[0.28em] text-iron-gold">
                  {b.label}
                </span>
                ...
                <figcaption className="mt-3 flex items-center justify-between">
                  ...
                  <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-iron-red/80">
                    {b.film}
                  </span>
                </figcaption>
              </figure>
        ...
        >
          ...
          <a
            href="#systems"
            className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-iron-red/20 bg-iron-red/5 px-5 py-2.5 font-mono text-[11px] font-medium uppercase tracking-[0.22em] text-foreground backdrop-blur-md transition-all duration-200 hover:bg-iron-red/10 active:translate-y-[1px]"
          >
            Open diagnostics
        ...
        {!loaded && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-5 bg-background px-6">
            <EyebrowBadge className="border-iron-red/30 text-iron-red bg-iron-red/5">TIMELINE // RESTORING</EyebrowBadge>
            <div className="h-px w-60 bg-white/10 md:w-80">
              <div
                className="h-full bg-iron-red transition-[width] duration-150 ease-out"
                style={{ width: `${Math.round(loadProgress * 100)}%` }}
              />
            </div>
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-zinc-500">
              Initializing &nbsp;&middot;&nbsp; {Math.round(loadProgress * 100)}%
            </p>
          </div>
        )}
      </div>
